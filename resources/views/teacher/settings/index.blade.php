...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('teacher.settings.update') }}" class="space-y-6">
        @csrf
        <div class="bg-white rounded-2xl border border-gray-100 p-6" style="box-shadow:0 2px 8px rgba(0,0,0,.06)">
            ...
            <div class="flex items-center gap-4 mb-6">
                <div class="h-16 w-16 rounded-full flex items-center justify-center text-white text-xl font-bold shrink-0" style="background-color:#2D9375;">{{ strtoupper(substr(auth()->user()->name, 0, 2)) }}</div>
                <div>
                    <p class="text-base font-bold text-gray-900">{{ auth()->user()->name }}</p>
                    <p class="text-sm text-gray-400">{{ auth()->user()->email }}</p>
                    <span class="inline-block mt-1 text-xs px-2.5 py-0.5 rounded-full font-semibold" style="background-color:#d1fae5; color:#065f46;">อาจารย์ / ที่ปรึกษา</span>
                </div>
            </div>
            <div class="space-y-4">
                <div>
        ...
        <div class="flex justify-end"><button type="submit" class="px-6 py-2 rounded-xl text-white font-medium" style="background-color:#2D9375;">💾 บันทึก</button></div>
    </form>
</div>
</x-app-layout>
