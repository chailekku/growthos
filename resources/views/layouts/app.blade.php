...
<head>
    ...
    <style>
        /* match reference site: nextjs/src/components/layout/Sidebar.tsx line 140 */
        #app-sidebar {
            position: fixed !important;
            top: 0 !important;
            left: 0 !important;
            width: 15rem !important;   /* w-60 */
            height: 100vh !important;  /* h-screen */
        ...
        @media (min-width: 768px) {
            ...
            #app-content { padding-left: 15rem !important; } /* md:pl-60 */
        }
    </style>
</head>
...
{{-- ══════════════════════════════════════════════════════════
     SIDEBAR — fixed left, desktop only (via CSS above)
══════════════════════════════════════════════════════════════ --}}
...
function navLink(string $href, string $label, string $iconName, bool $active): string {
    ...
    $dot  = $active ? '<span style="height:6px;width:6px;border-radius:9999px;background:white;flex-shrink:0;margin-left:auto;"></span>' : '';
...
@endphp
        ...
        @if($role === 'student')
            ...
            <div style="padding:0.5rem 0 0.25rem;"><div style="border-top:1px solid #c3e6cb;"></div></div>
...
{{-- ══════════════════════════════════════════════════════════
     MAIN CONTENT
══════════════════════════════════════════════════════════════ --}}
<div id="app-content" class="flex flex-col min-h-screen bg-gray-50">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.outside="open = false" x-transition
                    ...
                    <form method="POST" action="{{ route('demo.login') }}" class="divide-y divide-gray-100">
                        @csrf
                        <button type="submit" name="role" value="student" class="w-full text-left px-4 py-2.5 hover:bg-gray-50 text-sm {{ $role === 'student' ? 'font-semibold text-teal-700 bg-teal-50' : 'text-gray-700' }}">🎓 นักศึกษา</button>
                        <button type="submit" name="role" value="teacher" class="w-full text-left px-4 py-2.5 hover:bg-gray-50 text-sm {{ $role === 'teacher' ? 'font-semibold text-teal-700 bg-teal-50' : 'text-gray-700' }}">📚 อาจารย์</button>
                        <button type="submit" name="role" value="psychologist" class="w-full text-left px-4 py-2.5 hover:bg-gray-50 text-sm {{ $role === 'psychologist' ? 'font-semibold text-purple-700 bg-purple-50' : 'text-gray-700' }}">💜 นักจิตวิทยา</button>
                        <button type="submit" name="role" value="super_admin" class="w-full text-left px-4 py-2.5 hover:bg-gray-50 text-sm {{ $role === 'super_admin' ? 'font-semibold text-red-700 bg-red-50' : 'text-gray-700' }}">🛡️ ผู้ดูแลระบบ</button>
                    </form>
                </div>
            </div>
...
══════════════════════════════════════════════════════════════ --}}
@php
function mobileTab(string $href, string $label, string $iconName, bool $active): string {
    $color = $active ? '#2D9375' : '#9ca3af';
    $bg    = $active ? 'background:rgba(45,147,117,.10);border-radius:12px;' : '';
    $icon  = navIcon($iconName);
    return <<<HTML
<a href="{$href}"
   style="display:flex;flex-direction:column;align-items:center;justify-content:center;flex:1;color:{$color};text-decoration:none;padding:0.5rem 0;">
    <span style="{$bg} padding:4px 14px 3px;display:flex;flex-direction:column;align-items:center;gap:2px;">
        <svg style="height:1.25rem;width:1.25rem;flex-shrink:0;" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.75">{$icon}</svg>
        <span style="font-size:9.5px;font-weight:600;line-height:1;white-space:nowrap;">{$label}</span>
    </span>
</a>
HTML;
}
...
<nav style="display:none; position:fixed; bottom:0; left:0; right:0; z-index:50; background:white; border-top:1px solid #f3f4f6; box-shadow:0 -2px 16px rgba(0,0,0,.08); padding-bottom:env(safe-area-inset-bottom,0px);"
     id="mobile-tab-bar">
    <div style="display:flex; align-items:stretch; height:4rem; max-width:32rem; margin:0 auto;">

        @if($role === 'student')
            {!! mobileTab(route('student.dashboard'),        'หน้าหลัก',   'dashboard',  request()->routeIs('student.dashboard')) !!}
            {!! mobileTab(route('student.courses.index'),    'รายวิชา',    'courses',    request()->routeIs('student.courses.*')) !!}
            {!! mobileTab(route('student.ai-coach.index'),   'AI โค้ช',    'ai-coach',   request()->routeIs('student.ai-coach.*')) !!}
            {!! mobileTab(route('student.reflection.index'), 'สะท้อนคิด', 'reflection', request()->routeIs('student.reflection.*')) !!}
            {!! mobileTab(route('student.settings.index'),   'ตั้งค่า',    'settings',   request()->routeIs('student.settings.*')) !!}
        @elseif($role === 'teacher')
            {!! mobileTab(route('teacher.dashboard'),                  'หน้าหลัก',  'dashboard', request()->routeIs('teacher.dashboard')) !!}
            {!! mobileTab(route('teacher.student-analytics.index'),    'นักศึกษา',  'users',     request()->routeIs('teacher.student-analytics.*')) !!}
            {!! mobileTab(route('teacher.course-analytics.index'),     'รายวิชา',   'analytics', request()->routeIs('teacher.course-analytics.*')) !!}
            {!! mobileTab(route('teacher.settings.index'),             'ตั้งค่า',   'settings',  request()->routeIs('teacher.settings.*')) !!}
        @elseif($role === 'psychologist')
            {!! mobileTab(route('psychologist.dashboard'),                     'หน้าหลัก',  'heart',    request()->routeIs('psychologist.dashboard')) !!}
            {!! mobileTab(route('psychologist.emotional-trends.index'),        'อารมณ์',    'activity', request()->routeIs('psychologist.emotional-trends.*')) !!}
            {!! mobileTab(route('psychologist.support-recommendations.index'), 'คำแนะนำ',   'star',     request()->routeIs('psychologist.support-recommendations.*')) !!}
            {!! mobileTab(route('psychologist.settings.index'),                'ตั้งค่า',   'settings', request()->routeIs('psychologist.settings.*')) !!}
        @elseif($role === 'super_admin')
            {!! mobileTab(route('admin.dashboard'),                 'หน้าหลัก',  'dashboard', request()->routeIs('admin.dashboard')) !!}
            {!! mobileTab(route('admin.course-management.index'),   'รายวิชา',   'courses',   request()->routeIs('admin.course-management.*')) !!}
            {!! mobileTab(route('admin.users'),                     'ผู้ใช้',     'users',     request()->routeIs('admin.users')) !!}
            {!! mobileTab(route('admin.platform-analytics.index'),  'วิเคราะห์', 'analytics', request()->routeIs('admin.platform-analytics.*')) !!}
            {!! mobileTab(route('admin.settings.index'),            'ตั้งค่า',   'settings',  request()->routeIs('admin.settings.*')) !!}
        @endif
...
(function () {
    ...
    function check() {
        bar.style.display = window.innerWidth < 768 ? 'block' : 'none';
    }
    check();
